...
function openModal(sectionId) {
  ...
  if (templateContent) {
    ...
    // 1. Limpa as larguras especiais antes de abrir a nova aba
    modalContentBox.classList.remove('modal-largo', 'modal-medio');

    // 2. Aplica a largura correta dependendo da aba
    if (sectionId === 'projetos') {
      modalContentBox.classList.add('modal-largo'); // Usa 760px
    } else if (sectionId === 'sobre') {
      modalContentBox.classList.add('modal-medio'); // Usa 550px
    }
    // Se for 'skills' ou 'porto', fica sem classe extra e usa os 450px padrão!
